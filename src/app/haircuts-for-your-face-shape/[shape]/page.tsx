...
import { HAIRCUT_GUIDES, getHaircutGuide } from "@/lib/seo/haircutGuides";
import { searchLink } from "@/lib/shop/searchLink";

type Props = { params: Promise<{ shape: string }> };

export function generateStaticParams() {
  return HAIRCUT_GUIDES.map((g) => ({ shape: g.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { shape } = await params;
  const g = getHaircutGuide(shape);
  if (!g) return { title: "Haircut for your face shape — AuraCheck" };
  return {
    title: g.metaTitle,
    description: g.metaDescription,
    alternates: { canonical: `/haircuts-for-your-face-shape/${g.slug}` },
    openGraph: {
      title: g.metaTitle,
      description: g.metaDescription,
      type: "article",
      images: [{ url: `/api/og?category=${encodeURIComponent("Haircuts for " + g.shape + " Faces")}&leak=${encodeURIComponent("Free face-shape scan")}`, width: 1200, height: 630, alt: g.metaTitle }],
    },
  };
}

export default async function HaircutGuidePage({ params }: Props) {
  const { shape } = await params;
  const g = getHaircutGuide(shape);
  if (!g) notFound();

  const grooming = searchLink(g.stylingQuery, "amazon");
      ...
      <Container className="py-12">
        <article className="mx-auto max-w-2xl">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-[#1C1917] sm:text-4xl">
            The best haircuts for a {g.shapeLower} face
          </h1>
          ...
          <h2 className="mt-8 text-xl font-bold text-[#1C1917]">Best haircuts for a {g.shapeLower} face</h2>
          ...
          <h2 className="mt-8 text-xl font-bold text-[#1C1917]">Cuts to avoid</h2>
          <ul className="mt-3 space-y-3">
            {g.avoid.map((c) => (
          ...
          <div className="mt-8 rounded-2xl border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-5">
            <p className="text-sm font-semibold text-[#1C1917]">💈 How to style it</p>
            <p className="mt-1 text-sm leading-relaxed text-[#4a443d]">{g.styling}</p>
            <a
              href={grooming}
              target="_blank"
              rel="noopener noreferrer sponsored"
              className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-[#1c1917]/12 px-3 py-1.5 text-[11px] font-semibold text-[#1C1917] transition-colors hover:border-[#E14434]/40"
            >
              🧴 Shop styling products →
            </a>
          </div>

          <div className="mt-6 rounded-2xl border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-5">
          ...
          {/* Cross-links to sibling guides + the glasses guides for the same shape */}
          <div className="mt-10 border-t border-[#1c1917]/10 pt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {HAIRCUT_GUIDES.filter((o) => o.slug !== g.slug).map((o) => (
                <Link key={o.slug} href={`/haircuts-for-your-face-shape/${o.slug}`} className="rounded-full border border-[#1c1917]/10 bg-[#1c1917]/[0.03] px-3 py-1.5 text-xs font-medium text-[#4a443d] hover:border-[#E14434]/40 hover:text-[#1C1917]">
                  {o.shape} face haircuts
                </Link>
              ))}
            </div>
            <p className="mt-4">
              <Link href={`/glasses-for-your-face-shape/${g.slug}`} className="text-xs font-semibold text-[#B23A25] hover:underline">
                → And the best glasses for a {g.shapeLower} face
              </Link>
            </p>
          </div>

          <p className="mt-8 text-center text-[11px] text-[#9c9184]">
            AuraCheck may earn a commission on products bought through these links, at no extra cost to you.
          </p>
        </article>
      </Container>
    </>
