  ...
  return (
    <>
      ...
      <Container className="relative py-24">
        ...
        <h2 className="mb-2 text-lg font-bold text-[#1C1917]">Admin Access</h2>
        <p className="mb-6 text-xs text-[#857b6e]">Enter the admin code to access the panel. Server-side verified.</p>
        <input type="password" value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !loading && handleSubmit()} placeholder="Admin code" className="mb-3 w-full rounded-lg border border-[#1c1917]/10 bg-[#1c1917]/[0.04] px-3 py-2 text-sm text-[#1C1917] placeholder-gray-600 focus:border-red-500/50 focus:outline-none" />
...
export default function AdminPage() {
  ...
  const checklistItems = [
    ...
    { id: "aura_report", label: "Test Aura Report unlock (₹21)" },
  ...
  return (
    <>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Card><div className="text-xs text-[#857b6e]">Total Orders</div><div className="mt-1 text-2xl font-bold text-[#1C1917]">{orderStats?.totalOrders ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Payment Submitted</div><div className="mt-1 text-2xl font-bold text-amber-400">{orderStats?.paymentSubmitted ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Unlocked</div><div className="mt-1 text-2xl font-bold text-emerald-400">{orderStats?.unlockedOrders ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Cancelled</div><div className="mt-1 text-2xl font-bold text-red-400">{orderStats?.cancelledOrders ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Expected Revenue</div><div className="mt-1 text-2xl font-bold text-[#1C1917]">₹{orderStats?.totalExpectedRevenue ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Unlocked Revenue</div><div className="mt-1 text-2xl font-bold text-emerald-400">₹{orderStats?.totalUnlockedRevenue ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Total Audits</div><div className="mt-1 text-2xl font-bold text-[#1C1917]">{auditStats?.totalAudits ?? 0}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Total Unlocked</div><div className="mt-1 text-2xl font-bold text-red-400">{auditStats?.unlockedReports ?? 0}</div></Card>
      </div>

      {/* ─── Products Summary ─── */}
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Card><div className="text-xs text-[#857b6e]">Products</div><div className="mt-1 text-2xl font-bold text-[#1C1917]">{PRODUCTS.length}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Active</div><div className="mt-1 text-2xl font-bold text-emerald-400">{totalActive}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Sponsored</div><div className="mt-1 text-2xl font-bold text-amber-400">{totalSponsored}</div></Card>
        <Card><div className="text-xs text-[#857b6e]">Affiliate Clicks</div><div className="mt-1 text-2xl font-bold text-red-400">{affiliateStats?.totalClicks ?? 0}</div></Card>
      </div>

      {/* ─── Analytics ─── */}
      <Card className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[#1C1917]">Local Analytics</h3>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Total Events</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.totalEvents ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Free Scores</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.freeScoreGenerated ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Unlock Views</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.unlockPageViewed ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Payment Reqs</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.paymentRequestSaved ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Unlocks</div>
            <div className="mt-1 text-lg font-bold text-emerald-400">{analyticsSummary?.productUnlocked ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Share Downloads</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.shareCardDownloaded ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Affiliate Clicks</div>
            <div className="mt-1 text-lg font-bold text-amber-400">{analyticsSummary?.affiliateClicked ?? 0}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Printed</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{analyticsSummary?.reportPrinted ?? 0}</div>
          </div>
        </div>
        {(analyticsSummary?.freeScoreGenerated ?? 0) > 0 && (
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
              <div className="text-xs text-[#857b6e]">Payment Req / Free Score</div>
              <div className="mt-1 text-lg font-bold text-red-300">{analyticsSummary?.conversionEstimate.paymentRequestsPerFreeScore ?? "—"}</div>
            </div>
            <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
              <div className="text-xs text-[#857b6e]">Unlocks / Payment Req</div>
              <div className="mt-1 text-lg font-bold text-red-300">{analyticsSummary?.conversionEstimate.unlocksPerPaymentRequest ?? "—"}</div>
            </div>
          </div>
        )}
      </Card>
      ...
      {/* ─── Funnel Analytics ─── */}
      <Card className="mb-8">
        <h3 className="mb-4 text-sm font-semibold text-[#1C1917]">Product Funnel</h3>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Aura Report views</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{productPageViews.aura_report}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Dating Audit views</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{productPageViews.dating_audit}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Glow-Up Plan views</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{productPageViews.glowup_plan}</div>
          </div>
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-3">
            <div className="text-xs text-[#857b6e]">Examples views</div>
            <div className="mt-1 text-lg font-bold text-[#1C1917]">{productPageViews.examples}</div>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
            <div className="text-xs text-[#857b6e]">Aura Report unlocked</div>
            <div className="mt-1 text-lg font-bold text-emerald-400">{productUnlockCounts.aura_report}</div>
          </div>
          <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
            <div className="text-xs text-[#857b6e]">Dating Audit unlocked</div>
            <div className="mt-1 text-lg font-bold text-emerald-400">{productUnlockCounts.dating_audit}</div>
          </div>
          <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
            <div className="text-xs text-[#857b6e]">Glow-Up Plan unlocked</div>
      ...
      <Card className="mb-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-sm font-semibold text-[#1C1917]">Local Leads ({leads.length})</h3>
        ...
        {leads.length === 0 ? (
          <p className="text-sm text-[#857b6e]">No leads captured yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody>
                {leads.map((l) => (
                  <tr key={l.id} className="border-b border-[#1c1917]/[0.08] text-[#4a443d]">
                    ...
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e] max-w-[120px] truncate">{l.note || "—"}</td>
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e]">{formatDate(l.createdAt)}</td>
                    <td className="py-2">
      ...
      <Card className="mb-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-sm font-semibold text-[#1C1917]">Manual Orders ({orders.length})</h3>
          <div className="flex flex-wrap gap-2">
        ...
        {orders.length === 0 ? (
          <p className="text-sm text-[#857b6e]">No orders yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-b border-[#1c1917]/[0.08] text-[#4a443d]">
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e]">{o.id.slice(0, 8)}</td>
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e]">{o.auditId.slice(0, 8)}</td>
                    <td className="py-2 pr-3">{o.productName}</td>
                    <td className="py-2 pr-3 text-amber-400">
                      {o.discountAmount ? <span className="line-through text-[#9c9184] mr-1">₹{o.originalAmount || o.amount}</span> : ""}
                      ₹{o.amount}
                    </td>
                    <td className="py-2 pr-3">{o.discountCode ? <span className="text-emerald-400 text-[10px]">{o.discountCode} (-₹{o.discountAmount})</span> : <span className="text-[#9c9184]">—</span>}</td>
                    <td className="py-2 pr-3">
                      <Badge variant={STATUS_BADGE[o.status] || "default"}>{o.status.replace(/_/g, " ")}</Badge>
                    </td>
                    <td className="py-2 pr-3">
                      <div>{o.customerName || "—"}</div>
                      {o.customerContact && <div className="text-[10px] text-[#857b6e]">{o.customerContact}</div>}
                    </td>
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e]">{o.upiTransactionRef || "—"}</td>
                    <td className="py-2 pr-3">
                      {o.generatedUnlockCode ? (
                        <div className="flex items-center gap-1">
                          <span className="font-mono text-[10px] text-red-300">{o.generatedUnlockCode}</span>
                          <button onClick={() => handleCopyCode(o.generatedUnlockCode!)} className="text-[10px] text-[#857b6e] hover:text-red-300">Copy</button>
                        </div>
                      ) : (
                        <span className="text-[#9c9184]">—</span>
                      )}
                    </td>
                    <td className="py-2 pr-3 text-[10px] text-[#857b6e]">{formatDate(o.createdAt)}</td>
                    <td className="py-2">
                      <div className="flex flex-wrap gap-1">
                        {!o.generatedUnlockCode && (
                          <button onClick={() => handleGenerateCode(o)} className="rounded bg-red-500/20 px-1.5 py-0.5 text-[10px] text-red-300 hover:bg-red-500/30">
                            Generate Code
                          </button>
                        )}
                        ...
                        <button onClick={() => handleDeleteOrder(o.id)} className="rounded bg-[#1c1917]/[0.04] px-1.5 py-0.5 text-[10px] text-[#857b6e] hover:bg-[#1c1917]/[0.06]">
                          Delete
                        </button>
                      </div>
                    </td>
      ...
      {/* ─── Growth Dashboard ─── */}
      <Card className="mb-8">
        <h3 className="mb-4 text-sm font-semibold text-[#1C1917]">Growth Dashboard</h3>
        <div className="grid gap-6 sm:grid-cols-3">
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-4">
            <div className="mb-2 text-xs text-[#857b6e]">Referral Program</div>
            {(() => { const rs = getReferralStats(); return (
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span className="text-[#6f675e]">Referral Code</span><span className="text-red-300">{getReferralProfile()?.referralCode || "—"}</span></div>
                <div className="flex justify-between"><span className="text-[#6f675e]">Total Claims</span><span className="text-[#1C1917]">{rs?.totalClaimsLocal ?? 0}</span></div>
                <div className="flex justify-between"><span className="text-[#6f675e]">Referral Code Used</span><span className="text-[#1C1917]">{rs?.referralCode || "—"}</span></div>
          ...
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-4">
            <div className="mb-2 text-xs text-[#857b6e]">Challenge Entries</div>
            {(() => { const cs = getChallengeStats(); const entries = getChallengeEntries(); const uniqueChallenges = cs ? Object.keys(cs.entriesByChallenge).length : 0; return (
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span className="text-[#6f675e]">Total Entries</span><span className="text-[#1C1917]">{cs?.totalEntries ?? 0}</span></div>
                <div className="flex justify-between"><span className="text-[#6f675e]">Unique Challenges</span><span className="text-[#1C1917]">{uniqueChallenges}</span></div>
          ...
          <div className="rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.03] p-4">
            <div className="mb-2 text-xs text-[#857b6e]">Progress Comparisons</div>
            {(() => { const ps = getProgressStats(); const all = getProgressComparisons(); const improved = all.filter((c) => c.scoreDelta > 0).length; const deltas = all.map((c) => c.scoreDelta); const avgDelta = deltas.length > 0 ? Math.round(deltas.reduce((s, d) => s + d, 0) / deltas.length) : null; return (
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span className="text-[#6f675e]">Total Comparisons</span><span className="text-[#1C1917]">{ps?.totalComparisons ?? 0}</span></div>
                <div className="flex justify-between"><span className="text-[#6f675e]">Improved</span><span className="text-emerald-400">{improved}</span></div>
                <div className="flex justify-between"><span className="text-[#6f675e]">Avg Delta</span><span className="text-amber-400">{avgDelta != null ? `${avgDelta > 0 ? "+" : ""}${avgDelta}` : "—"}</span></div>
      ...
      {/* ─── Affiliate Stats ─── */}
      <Card className="mb-8">
        <h3 className="mb-4 text-sm font-semibold text-[#1C1917]">Affiliate Click Statistics</h3>
        {!affiliateStats || affiliateStats.totalClicks === 0 ? (
          <p className="text-sm text-[#857b6e]">No clicks recorded yet.</p>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <div className="mb-2 text-xs text-[#857b6e]">Clicks by Product</div>
              <div className="space-y-1.5">
                {Object.entries(affiliateStats.clicksByProduct).sort(([, a], [, b]) => b - a).map(([pid, count]) => {
                  const product = PRODUCTS.find((p) => p.id === pid);
                  return <div key={pid} className="flex items-center justify-between text-xs"><span className="text-[#4a443d]">{product?.title || pid}</span><span className="text-[#857b6e]">{count}</span></div>;
                })}
              </div>
            </div>
            <div>
              <div className="mb-2 text-xs text-[#857b6e]">Clicks by Source</div>
              <div className="space-y-1.5">
                {Object.entries(affiliateStats.clicksBySource).sort(([, a], [, b]) => b - a).map(([source, count]) => (
                  <div key={source} className="flex items-center justify-between text-xs"><span className="text-[#4a443d]">{source}</span><span className="text-[#857b6e]">{count}</span></div>
                ))}
              </div>
            </div>
          </div>
      ...
      {(() => {
        ...
        return (
          <Card className="mb-8">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-[#1C1917]">Founder Launch Checklist</h3>
              <span className="text-xs text-[#857b6e]">{completedCount}/{checklistItems.length} completed</span>
            </div>
            <div className="mb-3 h-2 overflow-hidden rounded-full bg-[#1c1917]/[0.04]">
              <div
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {checklistItems.map((item) => (
                <label key={item.id} className="flex cursor-pointer items-start gap-2 rounded-lg border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.02] p-2.5 hover:bg-[#1c1917]/[0.03]">
                  <input
                    type="checkbox"
                    checked={!!checklistState[item.id]}
                    onChange={() => toggleChecklistItem(item.id)}
                    className="mt-0.5 h-3.5 w-3.5 rounded border-[#1c1917]/20 bg-[#1c1917]/[0.04] text-red-600 focus:ring-red-500"
                  />
                  <span className={`text-xs ${checklistState[item.id] ? "text-[#857b6e] line-through" : "text-[#4a443d]"}`}>{item.label}</span>
                </label>
              ))}
            </div>
          </Card>
      ...
      {/* ─── Product Catalog ─── */}
      <Card className="mb-8">
        <h3 className="mb-4 text-sm font-semibold text-[#1C1917]">Product Catalog ({PRODUCTS.length})</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#1c1917]/[0.08] text-[#857b6e]">
            ...
            <tbody>
              {PRODUCTS.filter((p) => p.isActive).map((p) => (
                <tr key={p.id} className="border-b border-[#1c1917]/[0.08] text-[#4a443d]">
                  ...
                  <td className="py-2"><div className="flex flex-wrap gap-1">{p.goalTags.map((g) => (<span key={g} className="rounded-full bg-[#1c1917]/[0.04] px-1.5 py-0.5 text-[10px] text-[#857b6e]">{g}</span>))}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="flex items-center justify-between rounded-xl border border-[#1c1917]/[0.08] bg-[#1c1917]/[0.02] p-4 text-xs text-[#9c9184]">
        <span>⚠ Admin panel — data stored in localStorage. Server-side session auth.</span>
        <button onClick={handleLogout} className="rounded-lg border border-[#1c1917]/10 px-3 py-1.5 text-[#6f675e] hover:bg-[#1c1917]/[0.04] hover:text-[#1C1917] transition-colors">Logout</button>
      </div>
      </Container>
    </>
  );
