...
const ROWS: ComparisonRow[] = [
  ...
  },
  {
    ...
    glowupPlan: "Weekly check-in",
  },
  {
    label: "Full visual breakdown",
    free: false,
    auraReport: true,
    datingAudit: false,
    glowupPlan: false,
  ...
  {
    label: "Budget upgrade roadmap",
    free: "Basic",
    auraReport: "Detailed",
    datingAudit: "General",
    glowupPlan: "Full tiered",
  },
  {
...
function Cell({
  ...
  if (value === false) {
    return (
      <svg
        className="mx-auto h-5 w-5 text-[#9c9184]"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
          ...
          <thead>
            <tr className="border-b border-[#1c1917]/[0.08]">
              ...
              {HEADERS.map((h) => (
                <th
                  key={h.label}
                  className={`pb-3 px-2 text-center font-medium ${h.className || "text-[#857b6e]"}`}
                >
                  {h.label}
                </th>
              ))}
          ...
          <tbody>
            {ROWS.map((row) => (
              <tr key={row.label} className="border-b border-[#1c1917]/[0.08]">
                ...
                <td className="py-3 px-2 text-center">
                  <Cell value={row.auraReport} />
                </td>
                <td className="py-3 px-2 text-center">
                  <Cell value={row.datingAudit} />
                </td>
                <td className="py-3 px-2 text-center">
                  <Cell value={row.glowupPlan} />
                </td>
              </tr>
            ))}
          </tbody>
