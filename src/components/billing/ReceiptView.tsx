  ...
  return (
    <Card className="max-w-md mx-auto p-6">
      <div className="mb-4 text-center">
        <p className="text-xs text-[#857b6e] uppercase tracking-wider">Payment Receipt</p>
        <p className="mt-1 text-lg font-bold text-[#1C1917]">AuraCheck</p>
      </div>

      <div className="space-y-3 border-t border-[#1c1917]/10 pt-4 text-sm">
      ...
      <div className="mt-4 border-t border-[#1c1917]/10 pt-3 text-[10px] text-[#9c9184] text-center">
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between">
      <span className="text-[#857b6e]">{label}</span>
      <span className={highlight ? "font-medium text-amber-400" : "text-[#1C1917]"}>{value}</span>
    </div>
  );
}
